...
class FileUpload{
	...
	private function save2DB(){
		...
		$sharingMod = self::$appsdb->query("Select `sharingMod` From File_File where `idFile` = '$this->parentId';") 
						or $complete = false;
		if(!$complete)
			IOGate::reportError("Error While Saving data in database:\n");
		$sharingMod = $sharingMod->fetch(PDO::FETCH_ASSOC);
		$sharingMod = $sharingMod['sharingMod'] == "" ? "private" : $sharingMod['sharingMod'];
		if($complete){
			self::$appsdb->query("
				INSERT INTO `File_File`(`idUsers`, `idGroup`, `idApplication`, `name`,
								`parentDir`, `realPath`, `size`, `type`, `sharingMod`) 
				VALUES (
							'$this->userId','$this->groupId','$this->appId','$this->_fileName','$this->parentId',
							'$this->_folderPath','$this->_fileSize','$this->_fileExtention','$sharingMod');"
			) or $complete = false;
